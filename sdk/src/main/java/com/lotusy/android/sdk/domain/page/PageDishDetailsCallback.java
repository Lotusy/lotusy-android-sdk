 ...
 */
abstract public class PageDishDetailsCallback extends LotusyCallback {
    ...
    protected void doCallback(LotusyCallbackStatus status, JsonObject response) {
        ...
        if (status == LotusyCallbackStatus.SUCCESS) {
            try {
                dish = response.get("dish").getAsJsonObject();
        ...
        this.callback(result, dish);
    }

    abstract public void callback(LotusyTaskResult result, JsonObject dish);
}
